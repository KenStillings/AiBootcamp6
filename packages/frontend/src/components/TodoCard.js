...
function TodoCard({ todo, onToggle, onEdit, onDelete, isLoading }) {
  ...
  return (
    <div className={`todo-card ${todo.completed ? 'completed' : ''} ${isOverdue(todo) ? 'todo-overdue' : ''} ${isCompletedLate(todo) ? 'todo-completed-late' : ''}`}>
      <input
        type="checkbox"
      ...
      <div className="todo-content">
        <h3 className="todo-title">
          {isOverdue(todo) && <span className="overdue-icon">⚠️</span>}
          {todo.title}
        </h3>
        {todo.dueDate && (
